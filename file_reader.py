...
def get_parameters(file: str, num: int=0) -> Dict[str, Any]:
    ...
    with open(file, 'r') as file:
        ...
        for line in lines:
            if line.startswith('Q xxxxxxxxxxxxxxxxxxxx'):
                Q = float(line.split('/')[1])
            elif line.startswith('C xxxxxxxxxxxxxxxxxxxxx'):
                C = float(line.split('/')[1])
            elif line.startswith('g xxxxxxxxxxxxxxxxxxxxxxx'):
                g = float(line.split('/')[1])
            elif line.startswith('r xxxxxxxxxxxxxx'):
                h = float(line.split('/')[1])
            elif line.startswith('v xxxxxxxxxxxxxxx'):
